...
-   For a hospital with an average the patient stay of 0 days that conducts 0 xrays, the expected probability of infection is `r means[1]` (not realistic!).

-   Holding the xray ratio constant, we expect the probability of infection to increase by `r means[2]` for every one extra day in the hospital.

-   Holding the length of stay constant, we expect the probability of infection to increase by `r means[3]` for every one-unit increase in xray ratio.

-   Because the credible intervals for $\beta_{1}$ and $\beta_{2}$ exclude zero, I do think that they are both useful for helping us explain the probability of hospital infection.
